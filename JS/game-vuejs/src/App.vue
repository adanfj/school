...
<template>
  <div class="container">
    <div class="row g-0">
      <Entity v-for="entity in entities" :name="entity.name" :life="entity.life"
        :victory="entities.some(e => e.life <= 0 && e.name != entity.name)" />
      <button class="col mx-1 btn btn-outline-danger" @click="attack">Attack</button>
      <button :disabled="specialAttackBlock > 0" class="col mx-1 btn btn-outline-danger" @click="specialAttack">Special
        Attack</button>
    ...
    <div class="card mt-2">
      <div class="card-body">
        ...
        <div v-if="action.type == 'heal'"><span :class="action.attacker.name">{{ action.attacker.name }}</span> recovers
          <span class="text-success">{{ action.damage }}</span>
        </div>
        </p>
      </div>
    </div>
